...
class JsonSpec : StringSpec({
  ...
  "parses json to object" {
    val aPet = PetWrapper(
      mapOf(
        "pet" to mapOf(
          ...
          "dateOfBirth" to "2021-04-26".json(),
        ).json(),
      ).json(),
    )
    ...
    aPet.pet.dateOfBirth shouldBe LocalDate.of(2021, 4, 26)
  }
  ...
  "parses json to list of objects" {
      ...
      mapOf(
        "pets" to listOf(
          mapOf(
            "id" to "ba6e5573-06c0-4dd7-99ca-ef481fc2f870".json(),
            "name" to "Scamper".json(),
            "dateOfBirth" to "2021-04-25".json(),
          ).json(),
        ).json(),
      ).json(),
    )
    ...
    pets.pets.first().dateOfBirth shouldBe LocalDate.of(2021, 4, 25)
  }
})
