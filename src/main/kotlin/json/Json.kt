...
fun <V> ReadOnlyProperty<Any, Outcome<JsonInterpretationFailure, V>>.orThrow(): ReadOnlyProperty<Any, V> =
    ReadOnlyProperty { any, name -> getValue(any, name).orThrow { f -> if (f is Throwable) f else Exception(f.toString()) } }
...
sealed interface Json<T> {
    ...
    data class Object(
        ...
        fun <T> parseObject(f: (List<Pair<kotlin.String, Json<*>?>>) -> T): ReadOnlyProperty<Any, Outcome<JsonInterpretationFailure, T>> = obj().map { f(it.value) }

        fun array(key: kotlin.String): Outcome<FailedLookupResult, Array> = fetch<Array>(key)
        fun array() = readOnlyProperty { prop -> array(prop) }
        fun <T> parseArray(f: (List<Json<*>?>) -> T): ReadOnlyProperty<Any, Outcome<JsonInterpretationFailure, T>> = array().map { f(it.value) }
        ...
        fun <T> parseBoolean(f: (kotlin.Boolean) -> T): ReadOnlyProperty<Any, Outcome<JsonInterpretationFailure, T>> = boolean().map { f(it.value) }
        ...
        fun <T> parseNumber(f: (kotlin.Number) -> T): ReadOnlyProperty<Any, Outcome<JsonInterpretationFailure, T>> = number().map { f(it.value) }

        fun string(key: kotlin.String): Outcome<FailedLookupResult, String> = fetch<String>(key)
        fun string(): ReadOnlyProperty<Any, Outcome<FailedLookupResult, String>> = readOnlyProperty { prop -> string(prop) }
        fun <T> parseString(f: (kotlin.String) -> T): ReadOnlyProperty<Any, Outcome<JsonInterpretationFailure, T>> = string().map { f(it.value) }
        fun parseString(): ReadOnlyProperty<Any, Outcome<JsonInterpretationFailure, kotlin.String>> = parseString(::identity)
        ...
        fun replace(vararg entries: Pair<kotlin.String, Json<*>?>): Object =
            replace(entries.toList())

        @Suppress("UNUSED_PARAMETER")
        fun replace(entries: List<Pair<kotlin.String, Json<*>?>>): Object =
            TODO()
    }
